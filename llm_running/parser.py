...
def choice_answer_clean(pred: str):
    ...
    tmp = re.findall(r"\b(A|B|C|D|E|F|G)\b", pred.upper())
    if tmp:
        pred = tmp
    else:
...
def extract_answer(pred_str, data_name, use_last_number=True):
    ...
    if (
        data_name in ["mmlu_stem", "sat_math", "aqua", "xes_opt", "xes_pert_opt", "gaokao2023"]
        or data_name.startswith("xes_opt_ctx_")
    ):
    ...
    # choice answer
    if (
        data_name in ["sat_math", "aqua", "xes_opt", "xes_pert_opt"]
        or "mmlu" in data_name
        or data_name.startswith("xes_opt_ctx_")
    ):
        tmp = re.findall(r"\b(A|B|C|D|E|F|G)\b", pred.upper()) # Add options for xes (two data has up to G)
        if tmp:
            pred = tmp[-1]
        else:
            pred = pred.strip().strip(".")
...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    if data_name in ["math", "minerva_math"]:
        ...
        "xes_ctx_prereq_k2",
        "xes_ctx_prereq_k3",
        "xes_ctx_nonprereq_k1",
        "xes_ctx_nonprereq_k2",
        "xes_ctx_nonprereq_k3",
        "xes_ctx_allrandom_k1",
        "xes_ctx_allrandom_k2",
        "xes_ctx_allrandom_k3",
        "xes_ctx_prereq_v2_k1",
        "xes_ctx_prereq_v2_k2",
        "xes_ctx_prereq_v2_k3",
        "xes_ctx_nonprereq_v2_k1",
        "xes_ctx_nonprereq_v2_k2",
        "xes_ctx_nonprereq_v2_k3",
        "xes_ctx_allrandom_v2_k1",
        "xes_ctx_allrandom_v2_k2",
        "xes_ctx_allrandom_v2_k3",
        "xes_ctx_prereq_v3_k1",
        "xes_ctx_prereq_v3_k2",
        "xes_ctx_prereq_v3_k3",
        "xes_ctx_nonprereq_v3_k1",
        "xes_ctx_nonprereq_v3_k2",
        "xes_ctx_nonprereq_v3_k3",
        "xes_ctx_allrandom_v3_k1",
        "xes_ctx_allrandom_v3_k2",
        "xes_ctx_allrandom_v3_k3",
        "xes_ctx_prereq_final_k3",
        "xes_ctx_nonprereq_final_k3",
        "xes_ctx_allrandom_final_k3",
        "xes_opt_ctx_prereq_final_k3",
        "xes_opt_ctx_nonprereq_final_k3",
        "xes_opt_ctx_allrandom_final_k3",
        "xes_ctx_scaffold_k3",
        "xes_ctx_nonscaffold_k3",
        "xes_ctx_scaffold_allrandom_k3",
        "xes_opt_ctx_scaffold_k3",
        "xes_opt_ctx_nonscaffold_k3",
        "xes_opt_ctx_scaffold_allrandom_k3",
        "xes_ctx_sim_k3",
        "xes_ctx_simrandom_k3",
        "xes_opt_ctx_sim_k3",
        "xes_opt_ctx_simrandom_k3",
        "aime25",
        "aime24",
        "amc23",
        "cmath",
...
def parse_question(example, data_name):
    ...
    if data_name == "asdiv":
        ...
        options = example["options"] # ["A)4", "B)4.5", "C)5", "D)5.5", "E)6.5"]
        choice = "(" + "(".join(options)
        choice = choice.replace("(", " (").replace(")", ") ").strip()
        choice = "\nAnswer Choices: " + choice
        question = example["question"].strip() + choice
    elif "xes_opt" in data_name:
        options = example["options"] # "options": {"A": "odd", "B": "even", "C": "odd or even"}
        option_str = ""
        for k in sorted(options.keys()):
            option_str += f"({k}) {options[k]} "
        option_str = option_str.strip()
        question = example["question"].strip() + "\nAnswer Choices: " + option_str
    elif data_name == "gaokao_math_qa":
...
def parse_answer_choices_from_question(question: str) -> Dict[str, str]:
    """
    Parse choices from:
    'Answer Choices: (A) ... (B) ...'
    Return: {'A': '...', 'B': '...'}
    ...
    parts = question.split("Answer Choices:", 1)
    if len(parts) < 2:
        return {}

    choices_str = parts[1].strip()
    pattern = r"\(([A-G])\)\s*(.*?)(?=\s*\([A-G]\)\s*|$)"
    matches = re.findall(pattern, choices_str, flags=re.DOTALL)
...
def normalize_mcq_pred_to_letter(pred_str: str, question: str, data_name: str) -> str:
    """
    Return a choice letter (A-G) if possible.

    Order of preference (most specific first, to avoid mis-picking an isolated
    letter that happens to appear in the model's chain-of-thought):
      1) \\boxed{X} where X is itself a choice letter (e.g. \\boxed{B})
      2) \\boxed{X} where X (numerically/symbolically) matches one of the option
         values (e.g. \\boxed{200} when option C = 200)
      3) Fall back to choice_answer_clean, which looks for explicit 'answer is X'
         phrasing or a trailing isolated letter.
    """
    pred_str = pred_str or ""
    choices = parse_answer_choices_from_question(question or "")

    # 1) & 2) Prefer the boxed content if it cleanly resolves to a letter.
    if "boxed" in pred_str:
        cand = find_box(pred_str)
        cand = strip_string(cand, skip_unit=data_name in STRIP_EXCEPTIONS).strip()
        # (1) boxed contains a single choice letter
        if cand in list("ABCDEFG"):
            return cand
        # (2) boxed value matches an option's text after normalization
        if choices and cand:
            for ch, opt_text in choices.items():
                opt_norm = strip_string(opt_text, skip_unit=data_name in STRIP_EXCEPTIONS).strip()
                if opt_norm == cand:
                    return ch

    # 3) Fall back to legacy letter-in-text extraction
    return choice_answer_clean(pred_str)



def run_execute(executor, result, prompt_type, data_name, execute=False, question: str = ""):
    ...
    # NEW: if MCQ dataset, map boxed/value prediction -> letter using question choices
    if (
        data_name in ["mmlu_stem", "sat_math", "aqua", "xes_opt", "gaokao2023"]
        or data_name.startswith("xes_opt_ctx_")
    ):
        prediction = normalize_mcq_pred_to_letter(result, question or "", data_name)
