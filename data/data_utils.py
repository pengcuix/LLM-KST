...
if __name__ == '__main__':
    ...
    # XES
    # xes_dataset = XESDataset(data_dir=config.XES3G5M_DIR)
    nyt_standards = NYTMathStandards(file=config.NYS_MATH_STANDARDS)
    print(len(nyt_standards.domains))
    print(len(nyt_standards.clusters))
    # is_prerequisite = nyt_standards.poset.is_prerequisite('7', '106')
    # print(is_prerequisite)
    # for concept in nyt_standards.concepts:
    #     for pre_concept in nyt_standards.concepts[concept].pre_concepts:
    #         is_prerequisite = nyt_standards.poset.is_prerequisite(pre_concept, concept)
    #         print(concept, pre_concept, is_prerequisite)
    #
    # stu_questions = {stu_id: set(xes_dataset.interactions[stu_id].keys()) for stu_id in xes_dataset.interactions}
    #
    # selected_students = set()
    # selected_questions = set()
    # while len(selected_students) < len(stu_questions):
    #     max_size = 0
    #     selected = None
    #     for stu_id in stu_questions:
    #         if stu_id in selected_students:
    #             continue
    #         if not selected_questions:
    #             size = len(stu_questions[stu_id])
    #         else:
    #             size = len(stu_questions[stu_id] & selected_questions)
    #         if size > max_size:
    #             selected = stu_id
    #             max_size = size
    #
    #     selected_students.add(selected)
    #     if not selected_questions:
    #         selected_questions = selected_questions | stu_questions[selected]
    #     else:
    #         selected_questions = selected_questions & stu_questions[selected]
    #     print('n_stu: {}, n_questions: {}'.format(len(selected_students), len(selected_questions)))
    #     if len(selected_questions) < 100:
    #         break

    # xes_dataset.get_stats()
    # xes_dataset.plot_stu_ability()
    # xes_dataset.plot_item_difficulty()

    # NeurIPS
    # concepts
    # nyt_concepts = NYTMathStandards(file=config.NYS_MATH_STANDARDS)
    # # nyt_concepts.get_stats()
    # for concept_id, concept in nyt_concepts.concepts.items():
    #     print(concept.cluster_id)
    #
    # exit(1)
